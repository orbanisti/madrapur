...
class UrlsIterator extends \AppendIterator {
    ...
    protected function createPageLinksGenerator() {
        ...
        foreach ($models as $model) {
            /** @var Page $model */
            $url = new Url(\yii\helpers\Url::to([
                '/page/view',
                'slug' => $model->slug
            ], true), (new \DateTime())->setTimestamp($model->updated_at), Url::CHANGEFREQ_MONTHLY);
            yield $url;
        }
    }
    ...
    protected function createArticleLinksGenerator(): \Generator {
        ...
        foreach ($models as $model) {
            ...
            $url = new Url(\yii\helpers\Url::to([
                '/article/view',
                'slug' => $model->slug
            ], true), (new \DateTime())->setTimestamp($model->updated_at), Url::CHANGEFREQ_WEEKLY);
            yield $url;
        }
    }
}
