...
class ExtendedMessageController extends \yii\console\controllers\MessageController {
                ...
                $replacedSubject = preg_replace_callback(
                        ...
                        function ($matches) use ($newSourceLanguage, $fileName, &$unremoved) {
                            ...
                            if ($newSourceLanguage !== false) {
                                ...
                                $result = str_replace($message, Yii::t($category, $message, [], $newSourceLanguage),
                                        $matches[0]);
                            } else {
    ...
    protected function saveToDbOutput($messages, $config) {
        ...
        $db->createCommand()
            ->truncateTable($messageTable)
            ->execute();
        $db->createCommand()
            ->delete($sourceMessageTable)
            ->execute();
        ...
        foreach ($messages as $language => $categories) {
            ...
            $poFile->save(
                    FileHelper::normalizePath(
                            $config['messagePath'] . DIRECTORY_SEPARATOR . $language . DIRECTORY_SEPARATOR .
                            (isset($config['catalog']) ? $config['catalog'] : $this->catalog) . '.po'), $merged);
        }
    }
}
